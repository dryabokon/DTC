...
class Pipeliner:
    ...
    def get_hash(self,x):
        return hashlib.sha256(str(x).encode('utf-8')).hexdigest()
    # ----------------------------------------------------------------------------------------------------------------------
    def draw_detects(self, image, rects, labels=None, colors=(0, 128, 255),w=1):
        if labels is None:
        ...
        for rect, label,color in zip(rects, labels,colors):
            ...
            image = tools_draw_numpy.draw_line_fast(image, int(row_up),   int(col_left), int(row_up), int(col_right),color, w=w)
            image = tools_draw_numpy.draw_line_fast(image, int(row_down), int(col_left), int(row_down), int(col_right),color, w=w)
            image = tools_draw_numpy.draw_line_fast(image, int(row_up),   int(col_left), int(row_down), int(col_left),color, w=w)
            image = tools_draw_numpy.draw_line_fast(image, int(row_up),   int(col_right), int(row_down), int(col_right),color, w=w)
    ...
    # ----------------------------------------------------------------------------------------------------------------------
    def draw_debug_image(self,frame,df_track_frame_filtered):

        self.TP.tic('IO:draw')
        image_debug = tools_image.desaturate(frame, 0.3)
        if df_track_frame_filtered is None or df_track_frame_filtered.empty: return image_debug
        ...
        if (self.config.do_tracking is False) and (self.config.do_classification is False) and (self.config.do_profiling is False) and self.config.do_detection:
            image_debug = self.draw_detects(image_debug, df_track_frame_filtered[['x1', 'y1', 'x2', 'y2']].values,colors=(0, 128, 255))
        return image_debug
        # ----------------------------------------------------------------------------------------------------------------------
    def process_frame(self, filename,do_debug=True):
        ...
        if 'class_ids' not in df_track_frame.columns:
            df_track_frame['class_ids'] = numpy.nan

        if 'class_name' not in df_track_frame.columns:
            df_track_frame['class_name'] = numpy.nan

        if 'class_conf' not in df_track_frame.columns:
            df_track_frame['class_conf'] = numpy.nan

        if do_debug:
            image_debug = self.draw_debug_image(frame,df_track_frame_filtered)
            image_debug = self.print_debug_info(image_debug)
            image_debug = self.construct_timeline(frame.shape[1],df_track_frame_filtered)


        self.df_det_frame = df_track_frame
